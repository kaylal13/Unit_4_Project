...
public class PlainPoker {
    ...
    public void getRanks() {
        for (int i = 0; i < handCount; i++) {
            ...
            for (int j = 0; j < handCount; j++) {
                if (i != j && compareHands(hands[i], hands[j]) > 0) strongerCount++;
            }
    ...
    public void getRanksWild() {
        ...
        for (int i = 0; i < handCount; i++) {
            int strongerCount = 0;
            for (int j = 0; j < handCount; j++) {
                if (i != j && compareHandsWild(hands[i], hands[j]) > 0) strongerCount++;
            }
        ...
        int totalBidWild = 0;
        for (int i = 0; i < handCount; i++) totalBidWild += ranksWild[i] * bids[i];
    ...
    public void getTotalBid() {
        int totalBid = 0;
        for (int i = 0; i < handCount; i++) totalBid += ranks[i] * bids[i];
